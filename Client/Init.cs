...
public class Init : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        TextAsset config = Resources.Load<TextAsset>("ClientConfig");
        clientConfig = JsonUtility.FromJson<ClientConfig>(config.text);

        if (clientConfig.isLocal)
        {
            Debug.Log("on local model");

            gameObject.AddComponent<HotFix.InitEntity>();
        }
        else
        {
            Debug.Log("on online model");
    ...
    {
        if (!string.IsNullOrEmpty(clientConfig.serverInitUrl))
        {
            WWW result = new WWW(clientConfig.serverInitUrl);
            ...
            else
            {
                ...
                Version clientVersion = new Version(clientConfig.clientVersion);
                Version dllVersion = new Version(clientConfig.dllVersion);
